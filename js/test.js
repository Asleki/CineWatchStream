...
// API Configuration
// =====================
const TMDB_API_KEY = '9c198aabc1df9fa96ea8d65e183cb8a3'; // My TMDb API key
const OMDB_API_KEY = 'e195a3b0'; // My OMDB API key
...
import { showToast } from './CineWatch-main.js';
...
async function openTrailerModal(mediaType, showId) {
    ...
    if (videosData && videosData.results && videosData.results.length > 0) {
        ...
        if (allVideos.length === 1) {
            ...
            // Use .slice(0, 9) to display only the first 9 videos
            allVideos.slice(0, 9).forEach(video => {
...
// Function to check if the current show is in the playlist and update the button
const checkPlaylistStatus = (showId) => {
    const playlist = playlistManager.getPlaylist();
    const isInPlaylist = playlist.some(item => item.id.toString() === showId);
    if (isInPlaylist) {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        // Check and update playlist button visibility
        checkPlaylistStatus(showId);
        ...
        addToPlaylistBtn.addEventListener('click', async () => {
            ...
            const added = playlistManager.addItem(item);
            if (added) {
                showToast('Added to playlist successfully!');
                checkPlaylistStatus(showId);
            } else {
                showToast('This item is already in your playlist.');
            }
        });

        removeFromPlaylistBtn.addEventListener('click', () => {
            playlistManager.removeItem(showId);
            showToast('Removed from playlist successfully!');
            checkPlaylistStatus(showId);
        });
    ...
    // Inside the DOMContentLoaded event listener, after My other code...
